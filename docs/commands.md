...
| Command | Description |
| ------- | ----------- |
| ``disableanisched`` | Disable the anisched feature for this server
| ``goodbyetoggle`` | Toggle the Leaving Member Announcer on and off.
| ``setanischedch`` | Where will i post the announcement for recently aired anime?
| ``setgoodbyech`` | Set up the goodbye channel
| ``setgoodbyemsg`` | Set up the goodbye message. Supports Embeds!
| ``setsuggestch`` | Set up the suggestion channel
| ``setwelcomech`` | Set up the welcome channel
| ``setwelcomemsg`` | Set up the welcome message. Supports Embeds!
| ``unwatch`` | Removes a watched anime from your watchlist
| ``userxpreset`` | Reset the xp of a particular user in this server.
...
| Command | Description |
| ------- | ----------- |
| ``addemoji`` | Add an emoji to the server via the supplied link
| ``addroles`` | Adds the mentioned roles and/or supplied role IDs to the mentioned user
| ``ban`` | ban a user
| ``clear`` | Delete messages from this channel. Will not delete messages older than two (2) weeks
| ``hackban`` | bans a user, whether or not they are in the server.
| ``kick`` | kick mentioned user from your server
| ``lockdown`` | [Prevent/Allow] users from messaging in the current channel.
| ``mute`` | mute a user on all channels
| ``nuke`` | emoves all messages in the channel (Deletes the old channel and makes a copy of it with permissions intact)
| ``removeroles`` | Removes all applied roles to a user. (excludes default roles)
| ``respond`` | respond to a user suggestion
| ``setmute`` | Setup the mute role
| ``softban`` | Kicks a user and deletes all their messages in the past 14 days
| ``softlockdown`` | [Prevent/Allow] users without special permissions from sending messages in the current channel. Permission Overwrites will be kept.
| ``softmute`` | Toggle to prevent a user from sending a message in this channel
...
| Command | Description |
| ------- | ----------- |
| ``brackets`` | View or change a proxys brackets
| ``cfg`` | type `t!proxyhelp cfg` to get a help related to this command
| ``describe`` | View or change a proxys description
| ``export`` | Export your Proxy Data to a file
| ``findproxy`` | Attempts to find proxy with the given name
| ``group`` | Do `t!proxyhelp group` to get help related to this command
| ``import`` | Import Proxy Data from a exported file created by Tamako
| ``proxyavatar`` | View or Change Proxys Avatar
| ``proxybirthday`` | View or change a proxys birthday, or see upcoming birthdays
| ``proxyhelp`` | Get Help related Proxy(Using Webhooks to talk) aka multi account simulation
| ``proxylist`` | Get a detailed list of yours or another users registered proxys
| ``proxylistng`` | Get a detailed list of yours or another users registered proxys without groups
| ``proxyregister`` | Register A New Proxy; Do `t!proxyhelp proxyregister` to get detailed help
| ``proxyremove`` | Unregister a particular Proxy
| ``proxyrename`` | Change a Proxys name
| ``showuser`` | Show the user that registered the proxy that last spoke
| ``tag`` | Remove or change a proxys tag (displayed next to name when proxying)
| ``togglebrackets`` | Toggles whether the brackets are included or stripped in proxied messages for the given proxy
...
| ``nonxpchannels`` | See which channels do not give xp
...
Need to have xp system enabled in the guild, you can do that by doing `t!xptoggle`<br/>
[Click Here](./Setup/Point_System#economy-system) to see how to enable
:::
| Command | Description |
| ------- | ----------- |
| ``balance`` | Your wallet cant keep all your coins! So how about opening a bank account?
| ``bank`` | Your wallet cant keep all your coins! So how about opening a bank account?
...
| ``market`` | Check what you can buy from the shop
| ``previewitem`` | Check what you can buy from the shop
| ``profile`` | Shows the current xp, level, rank, and other details of a user
| ``register`` | Start earning currency. Register to keep track of your earned currencies!
| ``sell`` | Sell some of your items to the shop.
...
| ``aniquote`` | Generate a random anime quote
| ``anirandom`` | Generates a random anime recommendation. Recommends a Hentai if used on a nsfw channel
| ``book`` | Searches Google Books for a book.
| ``bulbapedia`` | Searches Bulbapedia for your query.
| ``character`` | Searches for a character in MAL
...
| ``hanime`` | Queries hanime.tv for a specific hentai. Returns a maximum of 10 results
...
| ``malprofile`` | Get user stats from MAL
| ``manga`` | Searches for a Manga from MAL
...
| ``nextairdate`` | Returns remaining time for the next episode of given anime. Returns this days schedule, if no anime is specified
...
| ``upcominganime`` | Displays the list of upcoming anime.
| ``urban`` | Search Something from urban
...
| ``hororscope`` | Find out what is your horoscope for today!
...
| ``mochizou`` | Responds with a random image of Mochizou.
...
| &nbsp;&nbsp;&nbsp;&nbsp;Command&nbsp;&nbsp;&nbsp;&nbsp; | Description |
| ------- | ----------- |
| ``mangarandom`` | Generates a random manga recommendation. You can add -discover parameter to generate a handpicked recommendations for a user
...
| ``give-flower`` | Gives Tamako Pai a flower.
| ``givepizza`` | Responds with a realistic pizza
| ``hi`` | Hello.
...
| ``volume`` | Change Volume of Music System of Tamako
| ``ytsearch`` | Search for a song on youtube
...
| ------- | ----------- |
| `4k` | 4k images
| `asian` | Fetch a image of asian hotties
| `asspic` | Fetch a image of ass
| `hentai` | Responds with a random hentai image.
| `porn` | Responds with a random porn image.
| `light-novel-cover` | Responds with a randomly generated Light Novel cover.
| `blowjobs` | BlowJobs
| `boobs` | Fetch a hot image of boobies
| `college` | Fetch a hot image of college girls
| `creampie` | Creampies
| `cumslut` | CumSluts
| `dickpic` | Fetch a hot image of dick
| `hot` | Fetch a random hot image
| `japanese` | Fetch a hot image of Japanese Girls
| `lesbian` | Fetch a hot image of Lesbain Girls
| `petite` | Fetch a hot image of Busty Petites
| `pussy` | Fetch a hot image of Pussy
