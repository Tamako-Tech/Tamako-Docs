...
| `t!economyreset` | Reset the economy of everyone in this server.
| `t!economytoggle` | Toggle the economy system on/off for the server.
| `t!goodbyetoggle` | Toggle the Leaving Member Announcer on and off.
| `t!setanischedch` | Where will i post the announcement for recently aired anime?
| `t!setgoodbyech` | Set up the goodbye channel
| `t!setgoodbyemsg` | Set up the goodbye message. Supports Embeds!
| `t!setsuggestch` | Set up the suggestion channel
| `t!setwelcomech` | Set up the welcome channel
| `t!setwelcomemsg` | Set up the welcome message. Supports Embeds!
| `t!unwatch` | Removes a watched anime from your watchlist
| `t!usereconomyreset` | Reset the economy of particular user in this server.
| `t!userxpreset` | Reset the xp of a particular user in this server.
| `t!watch` | Adds a new anime to watch for new episodes of.
| `t!welcometoggle` | Toggle the Joining Member Announcer on and off.
| `t!xpenable` | Enable collecting xp on Disabled mentioned channels
| `t!xpexcempt` | Disable collecting xp on mentioned channels
...
| `t!archive` | Archive a ticket. Sends the user a text transcript of the ticket and deletes the channel
| `t!archives` | Set server archives channel.
| `t!bind` | Bind the ticket starter reaction from a custom message
| `t!category` | Set server ticket category.
| `t!close` | Closes the ticket.Users in the ticket will still be able to read, but not send, messages. The channel will not be deleted.
| `t!config` | Configure ticket system settings.
| `t!delete` | Deletes the Ticket forever
| `t!description` | Changes a ticket Description. If no hid is given, edits the current channel ticket.
| `t!info` | Get info on a ticket.
| `t!list` | List open tickets.
| `t!modonly` | Set whether certain commands can only be used by mods. | Commands that can be set to mod only are add, remove, rename, description, close, and reopen.
| `t!open` | Re-open a closed ticket.
| `t!post` | Post a message that users can react to in order to open tickets
| `t!remove` | Remove users from a ticket
| `t!rename` | Changes a ticket name. If no hid is given, renames the current channel ticket
| `t!setup` | Set up the server for Ticket System by Tamako.
| `t!ticket` | Get all commands or help related to ticket support System
| `t!ticketlimit` | Set server limit for how many tickets users can have open at the same time(default is 5)
...
| `t!addroles` | Adds the mentioned roles and/or supplied role IDs to the mentioned user
| `t!ban` | ban a user
| `t!clear` | Delete messages from this channel. Will not delete messages older than two (2) weeks
| `t!hackban` | bans a user, whether or not they are in the server.
| `t!kick` | kick mentioned user from your server
| `t!lockdown` | [Prevent/Allow] users from messaging in the current channel.
| `t!mute` | mute a user
| `t!nuke` | emoves all messages in the channel (Deletes the old channel and makes a copy of it with permissions intact)
| `t!removeroles` | Removes all applied roles to a user. (excludes default roles)
| `t!respond` | respond to a user suggestion
| `t!setmute` | Setup the mute role
| `t!softban` | Kicks a user and deletes all their messages in the past 14 days
| `t!unban` | Unbans a user from this server
...
| `t!cfg` | type `t!proxyhelp cfg` to get a help related to this command
| `t!describe` | View or change a proxys description
| `t!export` | Export your Proxy Data to a file
| `t!findproxy` | Attempts to find proxy with the given name
| `t!group` | Do `t!proxyhelp group` to get help related to this command
| `t!import` | Import Proxy Data from a exported file created by Tamako
| `t!proxyavatar` | View or Change Proxys Avatar
| `t!proxybirthday` | View or change a proxys birthday, or see upcoming birthdays
| `t!proxyhelp` | Get Help related Proxy(Using Webhooks to talk) aka multi account simulation
| `t!proxylist` | Get a detailed list of yours or another users registered proxys
| `t!proxylistng` | Get a detailed list of yours or another users registered proxys without groups
| `t!proxyregister` | Register A New Proxy; Do `t!proxyhelp proxyregister` to get detailed help
| `t!proxyremove` | Unregister a particular Proxy
| `t!proxyrename` | Change a Proxys name
...
| `t!greroll` | Reroll the winners of a giveaway
| `t!gstart` | Start a giveaway
| `t!help` | Displays list ofall available commands, or detailed information for a specific command.
| `t!invite` | Responds with the bot's invite links.
| `t!leaderboard` | Shows the top xp earners for this server
| `t!mhelp` | Get all commands or help related to Counter System
| `t!nonxpchannels` | See which channels do not give xp
| `t!options` | Responds with a list of server options.
| `t!rank` | Shows the current xp, level, and rank of a user from the server if mentioned
| `t!remindme` | Make the bot remind you stuff
| `t!report` | Reports something to the bot owner(s).
| `t!schedule` | Displays the list of currently airing anime for todays date or given weekday.
...
| Command | Description |
| ------- | ----------- |
| `t!balance` | Your wallet cant keep all your coins! So how about opening a bank account?
| `t!bank` | Your wallet cant keep all your coins! So how about opening a bank account?
| `t!beg` | Want to earn money some more? Why dont you try begging
...
| `t!findmoney` | Find coins. Maybe we have some hidden around
| `t!register` | Start earning currency. Register to keep track of your earned currencies!
...
| `t!permissionsfor` | List the server permissions of mentioned user or provided ID
...
| `t!character` | Searches for a character in MAL
...
| `t!jisho` | Searches for Japanese words and Kanji on Jisho
...
| `t!malprofile` | Get user stats from MAL
| `t!manga` | Searches for a Manga from MAL
...
| `t!nextairdate` | Returns remaining time for the next episode of given anime. Returns this days schedule, if no anime is specified
...
| `t!pokemon` | Find a specific pokemon using the pokédex
| `t!pornhub` | Searches Pornhub for your query. (NSFW)
| `t!recipe` | Searches for recipes based on your query.
| `t!reddit` | Fetch a random image from supplied subreddit
...
| `t!urban` | Search Something from urban
...
| `t!birdfacts` | Generate a random useless bird facts
...
| `t!discover` | Generate a set of handpicked <Anime/Manga> recommendations for a user
...
| `t!hororscope` | Find out what is your horoscope for today!
...
| `t!pandafact` | Generate a random useless panda facts
...
| `t!animeme` | Generate an anime meme fetched from selected subreddits
...
| `t!img` | Generate a random image from selected categories
...
| `t!sabrina` | Get a picture of DA QUEEN
...
| `t!give-flower` | Gives Ship a flower.
| `t!givepizza` | Responds with a realistic pizza
| `t!hi` | Hello.
...
| `t!true-or-false` | Answer a true or false question.
| `t!typing-test` | See how fast you can type a sentence in a given time limit.
| `t!waldo` | Try to find Waldo with spoiler tags!
...
| `t!imposter` | Who is the imposter among us?
...
| `t!quiz-duel` | Answer a series of quiz questions against other opponents.
...
| `t!comment` | Comment something and return a youtube-like comment image
...
| `t!brony-speak` | Converts text to brony speak.
...
| `t!say` | Make me say what you want, master. (Owner-Only)
...
| `t!phone-block` | Gives instructions for blocking a channel or server.
...
| `t!asspic` | Fetch a image of ass
| `t!hentai` | Responds with a random hentai image.
| `t!porn` | Responds with a random porn image.
| `t!light-novel-cover` | Responds with a randomly generated Light Novel cover.
| `t!blowjobs` | BlowJobs
| `t!boobs` | Fetch a hot image of boobies
| `t!college` | Fetch a hot image of college girls
| `t!creampie` | Creampies
| `t!cumslut` | CumSluts
| `t!dickpic` | Fetch a hot image of dick
| `t!hot` | Fetch a random hot image
| `t!japanese` | Fetch a hot image of Japanese Girls
| `t!lesbian` | Fetch a hot image of Lesbain Girls
| `t!petite` | Fetch a hot image of Busty Petites
| `t!pussy` | Fetch a hot image of Pussy
...
| `t!respect` | Find a specific pokemon using the pokédex
